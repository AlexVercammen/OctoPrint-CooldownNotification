	...
	def on_api_command(self, command, data):
		if command == "testGCODE":
			if not self.inProgress:
				self._logger.debug("Testing GCODE")
...
#			notify_events = ['PrintFailed', 'PrintDone']		
			notify_events = ['PrintDone']		
			if event in notify_events:
				self._logger.debug("Received Event: " + event)
				self._logger.info("Print Ended, Watching HotEnd Temp")
	...
	##~~Custom Functions
	def checkTemp(self):
		if 'tool0' in self._printer.get_current_temperatures():
			toolTemp = self._printer.get_current_temperatures()['tool0']['actual']
			threshold = int(self._settings.get(["Threshold"]))
			self._logger.debug("Hot End Temp: " + str(toolTemp))
			if toolTemp <= threshold:
				self._logger.debug("Hot End Temp Reached Threshold")
				self._logger.debug("Hot End Temp: " + str(toolTemp) + "   Type: " + str(type(toolTemp).__name__))
...
# DISABLED tool0 not working (09/19/2021)
